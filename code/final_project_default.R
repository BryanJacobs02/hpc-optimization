...
readcounts_raw = read.table("featureCounts_PE.txt", header = TRUE, sep = "")
...
sample = gsub(".*STAR\\.([A-Za-z0-9]+)_Aligned.*", "\\1", names(readcounts_numeric))
...
ggsave("volcano_default.pdf", plot = volcano_plot, width = 8, height = 6)
...
writeLines(gene_list, "genes_for_gprofiler.txt")

# Import the geneID-symbol conversion table
conversion_table <- read.csv("converted_genes.csv")
...
write_gct(final_data, "output.gct")
...
writeLines(cls_final, "output.cls")